...
public class CookFoodsMaterialManageActivity extends BaseActivity implements OnClickListener {//
	...
	protected void onResume() {
		...
		if (myFoodMaterialCategoryListTemp == null || myFoodMaterialCategoryListTemp.size() == 0) {
			...
			for (int i = 0; i < myFoodMaterialCategoryListTemp.size(); i++) {
				List<LocalFoodMaterialLevelThree> list = FoodMaterialDAO.getAllFoodMaterialByCategoryId(myFoodMaterialCategoryListTemp.get(i).getId());
				if (list.size() > 0) {
					myFoodMaterialMap.put(myFoodMaterialCategoryListTemp.get(i).getId() + "", list);
					myFoodMaterialCategoryList.add(myFoodMaterialCategoryListTemp.get(i));
				}
			}
			...
			if (myFoodMaterialMap.size() == 0) {
				...
				if (myAdapter == null) {
					myAdapter = new MyCookFoodMaterialAdapter(context, myFoodMaterialCategoryList, myFoodMaterialMap, 1);
					expandLv_myFoodMaterial.setAdapter(myAdapter);
					expandLv_myFoodMaterial.setOnGroupClickListener(new OnGroupClickListener() {
						@Override
					...
					myAdapter.setData(myFoodMaterialCategoryList, myFoodMaterialMap);
				}
	...
	private void initView() {
		...
		if (layout_standardFoodMaterial.getVisibility() == View.GONE) {
			serach_food_name.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					Intent intent = new Intent(CookFoodsMaterialManageActivity.this, FoodssMangerSearchActivity.class);
		...
		rb_myFoodMaterial.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					rb_myFoodMaterial.setTextColor(Color.parseColor("#ff5b80"));
					...
					rb_myFoodMaterial.setTextColor(Color.parseColor("#3f3f3f"));
		...
		rb_standardFoodMaterial.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if (isChecked) {
					cancelManage();
					rb_standardFoodMaterial.setTextColor(Color.parseColor("#ff5b80"));
					view_standardFoodMaterial.setVisibility(View.VISIBLE);
					layout_standardFoodMaterial.setVisibility(View.VISIBLE);
					linear_search.setVisibility(View.GONE);
					linear_searchs.setVisibility(View.VISIBLE);
					btn_searchs.setOnClickListener(new OnClickListener() {
						@Override
						public void onClick(View v) {
							HHDLog.v("点击了onCheckedChanged-->onClick");
							Intent intent = new Intent(CookFoodsMaterialManageActivity.this, FoodsMangerSearchActivity.class);
							intent.putExtra("timeNode", timeNode);
							intent.putExtra("startTime", startTime);
							intent.putExtra("endTime", endTime);
							intent.putExtra("step", step);
								...
								new StandardFoodMaterialAdapter.OnReceiver() {
									...
									public void onClick(int groupPosition, int position, int ChildPosition) {
										if (position != -1) {
											...
											Intent intent = new Intent(context, StandardFoodsMaterialDetailsActivity.class);
											intent.putExtra("category", categoryFirstBeanList.get(groupPosition));
											...
											intent.putExtra("count", count);
											startActivityForResult(intent, 10258);
	...
	public void onClick(View v) {
		...
		case R.id.btn_addFoodMaterial:
			...
			final ArrayList<String> listId = (ArrayList<String>) myAdapter.getSelectedIds();
			final ArrayList<String> listUid = (ArrayList<String>) myAdapter.getSelectedIdsUid();
			i = count + listId.size();
			if (i <= 5) {
				if (listId.size() == 0) {
